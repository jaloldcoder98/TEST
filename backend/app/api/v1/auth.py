...
# auto-auth endpoints get more headroom since a legitimate client can hit them fairly often
# (token rotation, bot reconnects, reopening the Mini App) without that being suspicious.
...
_TELEGRAM_WEBAPP_AUTH_LIMIT = Depends(rate_limit("telegram_webapp_auth", max_requests=30, window_seconds=60))
...
@router.post("/telegram-webapp", response_model=TokenResponse, dependencies=[_TELEGRAM_WEBAPP_AUTH_LIMIT])
async def telegram_webapp_auth(data: TelegramWebAppAuthRequest, db: AsyncSession = Depends(get_db)) -> TokenResponse:
    return await auth_service.telegram_webapp_auth(db, data.init_data)
